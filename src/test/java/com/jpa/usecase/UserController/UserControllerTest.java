...
class UserControllerTest {
    ...
    @Test
    @DisplayName("UserAccountDeleteById")
    void deleteUserAccount(){

        Mockito.when(userService.deleteAccount(1L)).thenReturn(account);

        ResponseEntity<Account> userAccount = userController.deleteAccount(1L);

        verify(userService).deleteAccount(1L);

        assertEquals(HttpStatus.OK.value(),userAccount.getStatusCodeValue());
